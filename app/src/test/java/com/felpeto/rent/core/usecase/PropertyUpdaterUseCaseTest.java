...
class PropertyUpdaterUseCaseTest {

  private final Faker faker = new Faker();

  @Mock
  private UpdatePropertyPort updatePropertyPort;

  @InjectMocks
  private PropertyUpdaterUseCase propertyUpdaterUseCase;

  @Test
  void givenUuidAndPropertyWhenUpdateThenReturnProperty() {
    final var property = createProperty();
    final var uuid = UUID.randomUUID();

    when(updatePropertyPort.updateProperty(uuid, property)).thenReturn(property);

    final var response = propertyUpdaterUseCase.updateProperty(uuid, property);

    assertThat(response).isNotNull().isInstanceOf(Property.class);

    verify(updatePropertyPort).updateProperty(uuid, property);
    verifyNoMoreInteractions(updatePropertyPort);
  }
